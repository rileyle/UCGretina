...
Double_t fitf(Double_t *v,Double_t *par) {
  ...
  Double_t value = exp(par[0]+par[1]*v[0]) + exp(par[2]+par[3]*v[0])
    +  par[4]*sim0 +  par[5]*sim1 +  par[6]*sim2  +  par[7]*sim3;
...
void s44Fit(Int_t rebin){
  ...
  TString specFileName = "fakedata/fakedata.root";
  ...
  const Int_t nSim = 4;       // Number of simulated excited states
  TString simFileName[nSim];
  simFileName[0] = "s44_400_histos.root";
  simFileName[1] = "s44_480_histos.root";
  simFileName[2] = "s44_920_histos.root";
  simFileName[3] = "s44_1400_histos.root";
  ...
  const Int_t nPar = nSim+4;  // Number of fit parameters
