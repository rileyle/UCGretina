				    ...
				    G4bool isGamma)
{
  ...
  // LR: For now, we bypass any attempt by G4 at an angular distribution
  ...
  G4double theta = angDis->GetRandomAngle();                              // LR
  G4double cosTheta = std::cos(theta);                                    // LR
  G4double sinTheta = std::sin(theta);                                    // LR
  G4double phi = twopi * G4UniformRand();                                 // LR
  fDirection.set(sinTheta*std::cos(phi),sinTheta*std::sin(phi),cosTheta); // LR
