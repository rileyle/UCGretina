...
// Modified to include gamma-ray angular distribution coefficients
//
// Lew Riley
// 9 September 2016

#include "LevelReader.hh"
...
G4LevelReader::~G4LevelReader() 
{
  for(G4int i = 0; i < vGammaAngDist.size(); i++)
    delete vGammaAngDist[i];
}
...
G4LevelReader::MakeLevelManager(G4int Z, G4int A, const G4String& filename)
{
  ...
  if (!infile.is_open()) {
	...
	for(G4int i=0; i<10; ++i) { 
      ...
      if(isOK) {
	isOK = (isOK && (ReadDataItem(infile,fA0)));
	isOK = (isOK && (ReadDataItem(infile,fA2)));
	isOK = (isOK && (ReadDataItem(infile,fA4)));
      }
	...
	next = false;
	  ...
	  vGammaAngDist.push_back(new AngularDistribution(fA0, fA2, fA4));
	}
      }
      if(nline > 10000) {
