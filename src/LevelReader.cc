  ...
  : fData(ptr),fVerbose(0),fLevelMax(632),fTransMax(145)
{
  ...
  vGammaAngDist.resize(fTransMax,nullptr);
			    ...
			    std::ifstream& infile)
{
  ...
  for(i=0; i<nlevels; ++i) {
	...
	vGammaAngDist.resize(fTransMax);
      }
      fNorm1 = 0.0f;
	...
	if(!(ReadDataItem(infile,fA0)
	     && ReadDataItem(infile,fA2)
	     && ReadDataItem(infile,fA4))) {
	  if(fVerbose > 0) { 
	    G4cout << "### Fail to read angular distribution coefficients"
		   << " for transition j= " << j 
		   << G4endl; 
	  }
	  break;
	}
	vGammaAngDist[j] = new AngularDistribution(fA0, fA2, fA4);
      }
